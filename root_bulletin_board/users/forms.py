...
class RegisterUserForm(UserCreationForm):
    ...
    def clean_email(self):
        ...
        if get_user_model().objects.filter(email=email).exists():
            raise forms.ValidationError("Пользователь с таким E-mail'ом уже существует!")
        return email
...
class OneTimeCode(AuthenticationForm):
    code = forms.CharField(label="Одноразовый код",
                    widget=forms.TextInput(attrs={'class': 'form-input'}))
